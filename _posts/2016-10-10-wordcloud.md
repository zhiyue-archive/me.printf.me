...
##### `tf-idf`
谈起自动摘要算法首先想到的就是`tf-idf`算法，它的优点在于原理简单易于实现，同时效果也还可以。
TF-IDF的主要思想是，如果某个词或短语在一篇文章中出现的频率TF高，并且在其他文章中很少出现，则认为此词或者短语具有很好的类别区分能力，适合用来分类。TF词频(Term Frequency)指的是某一个给定的词语在该文件中出现的次数。IDF反文档频率(Inverse Document Frequency)的主要思想是：如果包含词条的文档越少，IDF越大，则说明词条具有很好的类别区分能力。使用`TF*IDF`可以计算某个关键字在某篇文章里面的重要性。使用`jieba`内置的`tf-idf`实现:
